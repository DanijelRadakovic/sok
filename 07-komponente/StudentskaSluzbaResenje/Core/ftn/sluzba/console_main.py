...
from ftn.sluzba.services.fakultet import (
    FakultetUcitatiBase,
    FakultetPrikazBase,
)

from ftn.sluzba.services.student import (
    StudentUcitatiBase,
    StudentPrikazBase
)


def console_menu(**kwargs):
    plugini: List[Union[
        FakultetUcitatiBase,
        FakultetPrikazBase,
        StudentUcitatiBase,
        ...
        kwargs.get("student_ucitavanje", []) + kwargs.get("student_prikaz", [])
    if not plugini:
...
def izabrana_opcija(
        plugin: Union[FakultetUcitatiBase, FakultetPrikazBase, StudentUcitatiBase, StudentPrikazBase],
        **kwargs
):
    try:
        if isinstance(plugin, FakultetUcitatiBase):
            ...
            return f"Ucitani fakulteti koristeci '{plugin.name()}' plugin\n"
        elif isinstance(plugin, FakultetPrikazBase):
            fakulteti = kwargs["fakulteti"]
            return plugin.prikazati_fakultete(fakulteti)
        elif isinstance(plugin, StudentUcitatiBase):
            studenti = kwargs["studenti"]
            pomocna_lista = plugin.ucitati_studente(kwargs.get("fakulteti", []))
            del studenti[:]
            studenti.extend(pomocna_lista)
            return f"Ucitani studenti korsteci '{plugin.name()}' plugin\n"
        elif isinstance(plugin, StudentPrikazBase):
            studenti = kwargs["studenti"]
            return plugin.prikazati_studente(studenti)
...
def load_plugins(oznaka):
    ...
    """
    plugins = []
    for ep in entry_points(group=oznaka):
        # Ucitavanje plagina.
        p = ep.load()
        print(f"{ep.name} {p}")
...
def main():
    try:
        ...
        student_ucitavanje = load_plugins("student.ucitavanje")
